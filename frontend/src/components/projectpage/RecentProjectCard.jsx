...
import {
  FiMoreHorizontal, FiStar, FiEye, FiEdit2, FiDownload, FiShare2, FiTrash2
} from "react-icons/fi";
import { FaRegStar, FaPalette, FaThLarge, FaGlobe } from "react-icons/fa";
...
const MenuItem = ({ icon, text, danger }) => (
  <div
    style={{
      ...
      cursor: "pointer", color: danger ? "#ed5972" : "#35345e",
      fontWeight: 600, fontSize: "1.04rem", borderTop: "1px solid #f2f0ff",
...
const RecentProjectCard = ({projects, hovered, setHovered, menuOpen, setMenuOpen }) => (
  <>
    {projects.map((data, i) => (
      <div
        key={i}
        style={{
          background: "#fff",
          borderRadius: 22,
          boxShadow: hovered === i ? "0 12px 48px #b1acf7cc" : "0 3px 17px #eaeaf93d",
      ...
      >
        <div style={{ position: "absolute", top: 20, right: 24, zIndex: 2 }}>
          {(hovered === i || menuOpen === i) && (
            <button
              ...
              style={{
                background: "#f5f6ff", border: "none", borderRadius: "50%",
                width: 36, height: 36,
        ...
        <div style={{ position: "absolute", top: 20, left: 24, zIndex: 1 }}>
          {data.favorite ? <FiStar size={22} color="#fdd835" title="Favorite" style={{ fill: "#fdd835" }} /> : <FaRegStar size={22} color="#e4e4e6" title="Not Favorite" />}
        </div>
        <div style={{ marginBottom: 16 }}>{data.icon}</div>
        <div style={{ fontWeight: 700, fontSize: "1.16rem", color: "#181d3a", marginBottom: 5 }}>{data.title}</div>
        <div style={{ color: "#757891", fontSize: "1.07rem", marginBottom: 16, minHeight: 38 }}>{data.desc}</div>
        <div style={{ marginBottom: 13, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <span style={{
            background: "#f7f8fb", color: "#322d4c", fontWeight: 600, borderRadius: 9, fontSize: "0.99rem", padding: "5px 16px"
          }}>{data.category}</span>
          <span style={{
            background: statusBg[data.status], color: statusColor[data.status], fontWeight: 700,
            borderRadius: 10, fontSize: "1.01rem", padding: "5px 15px"
          }}>{data.status}</span>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
          {data.hashtags.map((tag, idx) => (
            <span key={tag} style={{
              background: "#f5f6fc", color: "#888ca9", fontWeight: 600, borderRadius: 8,
              fontSize: "0.99rem", padding: "4px 11px"
            }}>{tag}</span>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontWeight: 500, color: "#adb2c0", fontSize: "1.07rem" }}>
          <span>{data.date}</span>
          <span>{data.size}</span>
        </div>
      </div>
