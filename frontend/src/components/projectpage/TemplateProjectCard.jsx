...
import { FaRegStar, FaGlobe, FaThLarge } from "react-icons/fa";
...
const MenuItem = ({ icon, text, danger }) => (
  <div
    style={{
      ...
      background: danger ? "#fff6f7" : "#fff", transition: "background 0.11s"
    }}
...
const TemplateProjectCard = ({ projects, hovered, setHovered, menuOpen, setMenuOpen }) => (
  <>
    {projects.map((data, i) => (
      <div
        key={i}
      ...
      >
        ...
        <div style={{ marginBottom: 13, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <span style={{
            background: "#f7f8fb", color: "#322d4c", fontWeight: 600, borderRadius: 9,
            fontSize: "0.99rem", padding: "5px 16px"
          }}>{data.category}</span>
          <span style={{
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
          {data.hashtags.map((tag, idx) => (
            <span key={tag} style={{
              background: "#f5f6fc", color: "#888ca9", fontWeight: 600,
              borderRadius: 8, fontSize: "0.99rem", padding: "4px 11px"
            }}>{tag}</span>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
          fontWeight: 500, color: "#adb2c0", fontSize: "1.07rem"
        }}>
          <span>{data.date}</span>
          <span>{data.size}</span>
        </div>
